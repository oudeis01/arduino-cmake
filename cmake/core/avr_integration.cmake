...
function(extract_avr_board_properties board_id boards_content)
    # Extract key properties for this board
    set(board_config)
    ...
    if(CMAKE_MATCH_1)
        ...
        list(APPEND board_config "VARIANT:standard")  # Default
    endif()
    ...
    if(CMAKE_MATCH_1)
        ...
        list(APPEND board_config "CORE:arduino")  # Default
    endif()
...
#-------------------------------------------------------------------------------
# ⚙️ AVR build configuration
#-------------------------------------------------------------------------------
function(apply_avr_build_config target_name)
    message(STATUS "⚙️  Applying AVR vendor build configuration...")
    ...
    message(STATUS "🎛️  Applied AVR configuration for ${board_mcu} @ ${board_f_cpu}")
endfunction()
...
#-------------------------------------------------------------------------------
# 🏗️ AVR post-build processing
#-------------------------------------------------------------------------------
function(execute_avr_post_build target_name)
    message(STATUS "🏗️  Executing AVR vendor post-build steps...")
    ...
    # Upload target (if avrdude is available)
    if(AVRDUDE)
...
function(collect_avr_sources)
    # Collect AVR vendor sources before target creation
...
function(create_avr_upload_target target_name)
    # Get board configuration for upload settings
    get_property(board_config CACHE AVR_BOARD_CONFIG_${TARGET_BOARD} PROPERTY VALUE)
    ...
    # Upload device (can be overridden)
